...
\begin{cvitems}
\item {Collected massive mechanical component CAD data with a web-scrapper, and expanded the data for dense data distribution using Creo Parametric and SolidWorks API. (\textit{Relevant publication: [\textbf{C5}]})}
\item {Developed a web-based 3D CAD search engine using a 3D Deep Neural Network and PostgreSQL database.}
\item {Benchmarked state-of-art 3D Deep Neural Network for classification and retrieval tasks to evaluate the dataset.}
\end{cvitems}
}
%------------------------------------------------
\cventry
...
{ % Description(s) of tasks/responsibilities
\begin{cvitems}
\item {Proposed a novel hand dataset by fusing RGB-D and thermal data for hand segmentation. (\textit{Relevant publication: [\textbf{C4}]})}
\item {Developed a method which predicts hand motion by segmenting hands and objects with a multi-modal Deep Neural network dealing with RGBD-T data.}
\end{cvitems}
}
